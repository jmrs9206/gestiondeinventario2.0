...
public class AuthService {
    ...
    public LoginResponse login(String email, String password, String ip, String userAgent) {
        ...
        if (!user.isAccountNonLocked()) {
            if (user.getLockoutUntil() != null && user.getLockoutUntil().isBefore(LocalDateTime.now())) {
                ...
                auditService.logEvent("User", user.getPublicId(), "LOGIN_FAILED", "USER", user.getPublicId(), ip, userAgent);
                throw new org.springframework.security.authentication.LockedException("La cuenta está bloqueada temporalmente por exceso de intentos fallidos.");
            }
        }
        ...
        String accessToken = jwtService.generateToken(user.getEmail(), user.getPublicId(), user.getRole().name(), user.isMustChangePassword());
        ...
        return new LoginResponse(accessToken, refreshTokenStr, expiresInSeconds, user.isMustChangePassword());
    }
    ...
    public LoginResponse refresh(String refreshTokenStr, String ip, String userAgent) {
        ...
        long expiresInSeconds = jwtService.getExpirationMs() / 1000;
        return new LoginResponse(accessToken, newRefreshTokenStr, expiresInSeconds, user.isMustChangePassword());
    }
